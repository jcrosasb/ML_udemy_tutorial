...
def find_split(data, obs, min_number):
    ...
    splits.append(split)
    return split_index, avg_left, avg_right


def recursive_split(data, obs, min_number):
    # Base case: Stop recursion when find_split returns None
    result = find_split(data, obs, min_number)
    if result is None:
        return None
    
    # Otherwise, continue recursively
    split_index, avg_left, avg_right = result

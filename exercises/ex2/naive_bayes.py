...
class NaiveBayes:
    ...
    def __init__(self, data, independent, dependent, size:float=0.2, seed:int=42) -> None:
        '''Initializer of NaiveBayes object.
    ...
    def _split_dataset_for_training(self, size:float, seed:int):
        '''Method to split datatset into training and testing. The method is ran
           automatically when a NaiveBayes object is created.
           Parameters:
                * size: (Integer) the proportion of the file that will 
                        be used for testing
...
if __name__ == '__main__':
    ...
    classifier = NaiveBayes(data=prep_df.sample(n=1000, random_state=42),
                                   independent=indep,
                                   dependent=dep)

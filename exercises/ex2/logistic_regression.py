...
class LogRegression:
    ...
    def __init__(self, data, independent, dependent, parameters={'C':1.0, 'solver':'lbfgs'}, \
                 size:float=0.2, seed:int=42) -> None:
        '''Initializer of LogRegression object.
           Attributes and parameters:
                ...
                * parameters: (Dict) LogisticRegression parameters. Default values are 
                              C and solver. See LogisticRegressor documentation for more information.
    ...
    def _split_dataset_for_training(self, size:float, seed:int):
        '''Method to split datatset into training and testing. The method is ran
           automatically when a LogRegression object is created.
           Parameters:
                * size: (Integer) the proportion of the file that will 
                        be used for testing
    ...
    def _train_model(self):
        '''Method to train model using method 'fit'. The method is ran
           automatically when a LogRegression object is created.'''
        self.trained_model = LogisticRegression(**self.parameters)
        self.trained_model.fit(self.X_train, self.y_train)
        self.y_pred = self.predict(self.X_test)


    def get_coefficients(self):
        '''Method to get the coefficients of the logistic regression
           Returns:
                * (Numpy array) coefficient of logistic regression'''
        return np.hstack((self.trained_model.intercept_[:, np.newaxis], self.trained_model.coef_))[0]
...
if __name__ == '__main__':
    ...
    # Instantiate LogRegression 
    regressor = LogRegression(data=prep_df.sample(n=10000, random_state=42),
                                   independent=indep,
                                   dependent=dep)

    # Instantiate Plotter
    plot = Plotter(x=regressor.X_test, 
                   y=regressor.y_test, 
                   xlabel=indep,
                   ylabel=dep).classifier(predictor=regressor.predict, 
                                          legends=['Normal', 'Fail'])
    
    eval = ModelEvaluator(x=regressor.X_test, 
                          y=regressor.y_test,
                          y_pred=regressor.y_pred).metrics(type='classifier', show=True)
